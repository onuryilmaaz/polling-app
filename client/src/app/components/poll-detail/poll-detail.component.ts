...
//         event.container.data.length >= maxSelections
//       ) {
//         // Maksimum seçim sayısını aşamaz
//         return;
//       }
...
//           // Drag-drop için güncellenen validasyon
//           if (question.maxSelections != null)
//             if (
//               !this.selectedRankingOptions[question.id] ||
//               this.selectedRankingOptions[question.id].length <
//                 question.maxSelections
//             ) {
//               alert(
//                 `Lütfen "${question.text}" sorusunda ${question.maxSelections} adet seçeneği sıralayınız.`
//               );
//               return false;
//             }
//           break;
        ...
        if (this.poll.questions) {
          this.poll.questions.forEach((question) => {
            ...
            if (question.type === QuestionType.Ranking && question.options) {
              ...
              // Drag-drop için yeni yapıları başlat
              this.selectedRankingOptions[question.id] = [];
              this.unselectedRankingOptions[question.id] = [...question.options];
            }
          });
        }
        this.checkSubmissionStatus(pollId);
    ...
    for (const question of this.poll.questions) {
      ...
      switch (question.type) {
        ...
        case QuestionType.Ranking:
          ...
          if (maxSelections === 0 && this.selectedRankingOptions[question.id].length === 0) {
            alert(
